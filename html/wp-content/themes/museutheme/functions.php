	...
	*/
	...
	if ( is_singular() )
	{
		...
		*/
		...
		if ( 'object' === $post_type )
		{
			...
			$categories = wp_get_object_terms( $post_id, 'categoria',  array('orderby' => 'name', 'order' => 'ASC', 'fields' => 'all'));
			...
			if ( $categories ) {
				...
				do{
					...
					$category  = get_term( $cat_parent,'categoria');

				} while($category->parent);
				$all_categories[] = $category;
				$all_categories = array_reverse($all_categories);
				?>
				<div class="wrapper">
				...
				echo "<li class='last active'><a href='";
	...
	if( is_archive() )
	{
		...
		if($category->parent){
			...
			$last_removed = array_pop($all_categories);
			?>
			<div class="wrapper">
			  <ul class="breadcrumbs">
				<li class="first"><a href="/" class="icon-home"></a>Home</li>
			...
			echo "<li class='last active'><a href='";
			echo  get_term_link( $last_removed );
			echo "'>";
			echo $last_removed->name;
			echo "</a></li>";
			?>
			</ul>
			</div>
